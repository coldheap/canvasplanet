...
export function registerPaintRoutes(app: FastifyInstance): void {
  app.post<{ Body: PaintRequest }>("/api/paint", async (req, reply) => {
    ...
    // event waits for registered zone writes, then refuses new ones until
    // rollback and rectangle removal are complete.
    const finishEventPaint = events.beginPlayerPaint(x, y);
    if (!finishEventPaint) {
      return reply.code(REFUSAL_STATUS[PaintRefusal.EventResolving]).send({
        ok: false,
        reason: PaintRefusal.EventResolving,
        message: REFUSAL_MESSAGE[PaintRefusal.EventResolving],
